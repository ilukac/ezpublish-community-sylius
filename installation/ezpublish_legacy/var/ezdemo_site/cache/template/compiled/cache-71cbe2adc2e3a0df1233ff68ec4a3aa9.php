...
$oldSetArray_663c4ba4157e78e763ab33e03a034bdf = isset( $setArray ) ? $setArray : array();
$setArray = array();
$tpl->Level++;
if ( $tpl->Level > 40 )
{
...
$text .= ']</span> Content view cache was cleared</h2>
    </div>';
}
unset( $if_cond );
// if ends
...
$text .= ']</span> All caches were cleared</h2>
    </div>';
}
unset( $if_cond );
// if ends
...
$text .= ']</span> Ini file cache was cleared</h2>
    </div>';
}
unset( $if_cond );
// if ends
...
$text .= ']</span> Template cache was cleared</h2>
    </div>';
}
unset( $if_cond );
// if ends
...
$text .= ']</span> Static content cache was regenerated</h2>
    </div>';
}
unset( $if_cond );
// if ends
...
$text .= ']</span> The following caches were cleared:</h2>
        <ul>
        ';
unset( $loopItem );
unset( $loopItem );
...
<form name="clearcacheform" method="post" action="/administration/setup/cache">
...
<h1 class="context-title">Clear caches</h1>
...
<tr>
    <th width="61%">Categories</th>
    <th width="39%"></th>
</tr>


<tr class="bglight">
<td>Template overrides and compiled templates:</td>
<td><input class="button" type="submit" name="ClearTemplateCacheButton" value="Clear template caches" title="This operation will clear all the template override caches and the compiled templates. It may lead to slower site performance until the caches are recreated." /></td>
</tr>


<tr class="bgdark">
<td>Content views and template blocks:</td>
<td><input class="button" type="submit" name="ClearContentCacheButton" value="Clear content caches" title="This operation will clear all caches that are related to either template views or cache blocks inside the pagelayout template. Use it if you have modified templates or if you have made changes inside a cache block."/></td>
</tr>


<tr class="bglight">
<td>Configuration (ini) caches:</td>
<td><input class="button" type="submit" name="ClearINICacheButton" value="Clear Ini caches" title="This operation will clear all the configuration caches. Use it to force the system to re-read the configuration files if you have changed settings." /></td>
</tr>


<tr class="bgdark">
<td>Everything:</td>
<td><input class="button" type="submit" name="ClearAllCacheButton" value="Clear all caches" title="This operation will clear all the caches and may lead to slow site response times until the caches are recreated." /></td>
</tr>
...
<h2 class="context-title">Fine-grained cache control</h2>
...
<tr>
    <th class="tight"><img src="/design/admin/images/toggle-button-16x16.gif" width="16" height="16" alt="Invert selection." onclick="ezjs_toggleCheckboxes( document.clearcacheform, \'CacheList[]\' ); return false;" title="Invert selection." /></th>
    <th>Name</th>
    <th>Path</th>
</tr>';
unset( $loopItem );
...
$var1 = strtr( 'Select the <%cache_name> for clearing.', $var4 );
...
$var1 = strtr( 'The <%cache_name> is disabled and thus it cannot be marked for clearing.', $var4 );
unset( $var3, $var4, $var5, $var6 );
if (! isset( $var1 ) ) $var1 = NULL;
while ( is_object( $var1 ) and method_exists( $var1, 'templateValue' ) )
    $var1 = $var1->templateValue();
...
<div class="block">
<input class="button" type="submit" name="ClearCacheButton" value="Clear selected" title="Clear the selected caches." />
</div>
</div></div>
</div>
...
<h2 class="context-title">Static content cache</h2>
...
$setArray = $oldSetArray_663c4ba4157e78e763ab33e03a034bdf;
$tpl->Level--;
?>
