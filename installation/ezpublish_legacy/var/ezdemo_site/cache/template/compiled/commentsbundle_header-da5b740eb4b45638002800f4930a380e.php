<?php
// URI:       design:tabs/commentsbundle_header.tpl
// Filename:  extension/ezcommentsbundle/design/admin/templates/tabs/commentsbundle_header.tpl
// Timestamp: 1415096122 (Tue Nov 4 11:15:22 CET 2014)
$oldSetArray_c6dd62b54036122dfdd911f495d0713c = isset( $setArray ) ? $setArray : array();
$setArray = array();
$tpl->Level++;
if ( $tpl->Level > 40 )
{
...
$text .= '<li id="node-tab-commentsbundle" class="';
// if begins
unset( $if_cond );
unset( $if_cond );
...
$if_cond = ( ( $if_cond1 ) == ( 'commentsbundle' ) );
unset( $if_cond1 );
if (! isset( $if_cond ) ) $if_cond = NULL;
while ( is_object( $if_cond ) and method_exists( $if_cond, 'templateValue' ) )
    $if_cond = $if_cond->templateValue();
...
if ( $if_cond )
{
$text .= '        <span class="disabled" title="Tab is disabled, enable with toggler to the left of these tabs.">Comments</span>
    ';
}
else
{
...
$text .= ' title="Content comments overview.">Comments</a>
    ';
}
unset( $if_cond );
// if ends
...
$setArray = $oldSetArray_c6dd62b54036122dfdd911f495d0713c;
$tpl->Level--;
?>
